...
import { 
  ...
  PieChart, Target, CalendarDays, Maximize2, Trophy, Lightbulb, User, TrendingDown, Medal, Eye, Clock, Crown, Download
} from 'lucide-react';
import { Invoice } from '@/hooks/useInvoices';
import { formatCurrency, formatNumber } from '@/lib/formatters';
import { format, startOfMonth, endOfMonth, isWithinInterval, subMonths, addMonths, startOfYear, endOfYear, getYear, isSameDay, getMonth } from 'date-fns';
...
export const Statistics = ({ invoices, sellerName, onPreviewInvoice }: StatisticsProps) => {
  ...
  const smartAnalysis = useMemo(() => {
    ...
    if (selectedMonthStats.invoiceCount === 0) {
      ...
      if (secondPlace && secondPlace.commission > 0) {
        narrative += `Le sigue "${secondPlace.name}" con una sólida contribución, ya que vendió $${formatNumber(secondPlace.sales)} para DLS, de los cuales ${firstName} comisionó $${formatNumber(secondPlace.commission)}. `;
      }
        ...
        {viewMode === 'month' ? (
            <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
                <Card className={`col-span-1 md:col-span-2 text-white border-none shadow-lg relative overflow-hidden bg-gradient-to-br ${commChange.isPositive ? 'from-emerald-600 to-emerald-500' : 'from-rose-600 to-rose-500'}`}>
                    ...
                    <CardHeader className="pb-2"><CardTitle className="text-base font-bold text-white/90 uppercase tracking-wide flex items-center gap-2"><Target className="h-5 w-5" /> Comisiones de {sellerName || 'Usuario'}</CardTitle></CardHeader>
                    <CardContent>
                ...
                <Card className="col-span-1 md:col-span-2 border border-border/50">
                    ...
                    <CardContent className="flex flex-col gap-4 pt-2">
                        ...
                        <div className="flex items-center justify-between p-4 bg-white dark:bg-card rounded-lg border border-border shadow-sm">
                            <div><div className="text-xs text-muted-foreground uppercase font-bold mb-1 flex items-center gap-1"><Badge variant="outline" className="h-5 px-1.5 text-[10px] border-slate-300">#2</Badge> Segundo Lugar</div><div className="font-bold text-foreground text-lg truncate max-w-[200px]">{smartAnalysis.secondPlace?.name || "-"}</div><div className="text-xs text-muted-foreground mt-1">Venta DLS: <span className="font-semibold text-foreground">${formatNumber(smartAnalysis.secondPlace?.sales || 0)}</span></div></div><div className="text-right"><div className="text-xs text-muted-foreground font-medium">Comisión {firstName}</div><div className="font-bold text-slate-600 dark:text-slate-300 text-xl">${formatNumber(smartAnalysis.secondPlace?.commission || 0)}</div></div>
                        </div>
                    ...
                    <div className="space-y-3 pt-2">
                        {selectedDayInvoices.map((inv) => (
                            <div key={inv.id} className="p-4 border rounded-lg bg-card shadow-sm hover:border-primary/50 transition-colors">
                                <div className="flex justify-between items-start mb-3"><div className="flex flex-col"><span className="font-mono font-bold text-base text-foreground">{inv.ncf}</span></div><div className="text-right"><div className="text-xs text-muted-foreground mb-0.5">Comisión:</div><div className="font-bold text-emerald-600 text-lg">${formatCurrency(inv.total_commission || 0)}</div></div></div>
            ...
            <DialogContent className="max-w-md">
                ...
                {recordInvoice && (
                    <div className="p-4 border rounded-lg bg-card shadow-sm mt-2">
                        <div className="flex justify-between items-start mb-3"><div className="flex flex-col"><span className="font-mono font-bold text-xl text-foreground">{recordInvoice.ncf}</span></div><div className="text-right"><div className="text-xs text-muted-foreground mb-0.5">Comisión Total</div><div className="font-bold text-emerald-600 text-2xl">${formatCurrency(recordInvoice.total_commission || 0)}</div></div></div>
