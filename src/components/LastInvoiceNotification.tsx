...
const Index = () => {
  ...
  const handleSaveInvoice = async (ncf: string, invoiceDate: string) => {
    ...
    if (result) {
      ...
      // RESTAURADO: Notificación de última factura guardada
  ...
  // Helper para renderizar el contenido del diálogo de factura
    ...
    <div className="space-y-6 pt-2">
      ...
      {invoice.products && invoice.products.length > 0 && (
         <div className="space-y-2">
            <h4 className="text-sm font-semibold text-muted-foreground uppercase tracking-wider">Productos</h4>
            <div className="border rounded-lg divide-y text-sm">
                {invoice.products.map((prod, idx) => (
                    <div key={idx} className="flex justify-between p-3">
                        <span>{prod.product_name}</span>
                        <div className="text-right">
                            <div className="font-medium">${formatNumber(prod.amount)}</div>
                            <div className="text-xs text-muted-foreground text-emerald-600 font-medium">
                                +${formatCurrency(prod.commission)} ({prod.percentage}%)
                            </div>
                        </div>
                    </div>
                ))}
                {invoice.rest_amount > 0 && (
                    <div className="flex justify-between p-3 bg-slate-50 dark:bg-slate-900/50">
                        <span>Resto de productos</span>
                        <div className="text-right">
                            <div className="font-medium">${formatNumber(invoice.rest_amount)}</div>
                            <div className="text-xs text-muted-foreground text-emerald-600 font-medium">
                                +${formatCurrency(invoice.rest_commission)} ({invoice.rest_percentage}%)
                            </div>
                        </div>
                    </div>
                )}
  ...
  return (
    <div className="min-h-screen bg-background">
      ...
      {/* --- Dialogo de Preview (Click manual) --- */}
      ...
      {/* --- Dialogo de Notificación de Factura Guardada (Automático) --- */}
      <Dialog open={!!lastInvoiceNotification} onOpenChange={(open) => !open && setLastInvoiceNotification(null)}>
        <DialogContent className="max-w-md border-emerald-500 border-2">
          ...
          {lastInvoiceNotification && renderInvoiceDialogContent(lastInvoiceNotification)}
        </DialogContent>
      </Dialog>
    </div>
  );
